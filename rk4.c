...
#define G_K   80.0
#define G_M   1.45				// Was previously almost always 2, McCarthy paper says "between 0 and 4", gmi
#define G_L   0.1
...
static char doc[] =
  "an lts simulation programmed by Conrad Leonik, designed by Carmen Canavier\n";
...
static struct argp_option options[] = {
	...
	{"onlyprc",	'o', "ONLYPRC",		OPTION_ARG_OPTIONAL, "toggles doing only PRC, does nothing if --prc=CLUSTERSIZE not enabled"},
	...
	for (i = 0; i < nn; i++) {
		...
		if (self_connection) {
			...
			if (pertmode) {
				...
				dydx[P] = 2 * (1 + tanh((-50.0) / 4.0)) * (1 - y[P]) - y[P] / tau;	//not sure why THRESHOLD is being used here?
			}
		}
		else {
...
void pertsim(double normalperiod, Template spike, Phipair *trace, int tracedata, const char* tracename) {
	...
	double time, flag1, flag2, temp_flag;	//added temp_flag1 to allow 2-factor authentication for spikes
	...
	//flag2 = 0.0;
	temp_flag = 0.0;
	double flags[10];//spiketimes for the PRC
	int flagnum = 0;//increments the number of spikes the neuron has to put it in correct position of flags, shouldnt exceed 9
	flag = False;
	pertmode = False;
	for (i = 0; i < N; ++i) {
			dv[i] = 0.0;
